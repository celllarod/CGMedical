...
public class FarmacosSecundariosStepViewModel extends ViewModel {

    private MutableLiveData<Map<String, UUID>> nombresFarmacosMap;
    private MutableLiveData<FarmacoDetalle> farmaco;
    private MutableLiveData<List<FarmacoSecundario>> fsList;

    public FarmacosSecundariosStepViewModel(){  }
    ...
    public MutableLiveData<List<FarmacoSecundario>> getFsList() {
        return fsList;
    }

    public void addFsItemList(FarmacoSecundario fs) {
        if(Objects.isNull(this.fsList)){
            this.fsList = new MutableLiveData<>();
        }
        List<FarmacoSecundario> farmacoSecundarioList = new ArrayList<>();
        if(!Objects.isNull(fsList.getValue())) {
            farmacoSecundarioList.addAll(fsList.getValue());
        }
        farmacoSecundarioList.add(fs);
